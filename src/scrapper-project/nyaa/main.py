...
def get_user_link_from_view(html):
    try:
        ...
        if submitter is None:
            ...
            raise Exception("submitter is Anonymous, Skipped")
        submitter_url = submitter.get("href")  # type: ignore
        if submitter_url is None:
            raise Exception("submitter_URL isn't available")
...
def extract_nyaa_page_data(html, page_url):
    try:
        # Return data based on path: index, /user, /view
        if get_url_path(page_url, 0) in ['', '/', '/user']:
            return extract_nyaa_table_info(html)
        elif get_url_path(page_url, 0) == '/view':
            return get_user_link_from_view(html)
        else:
            message = f"path outside of scope encountered, url: {page_url}"
            line_print(Back.YELLOW, message)
...
class AsyncScraper:
    ...
    async def extract_table_page(self, url_to_crawl, page_num):
        ...
        try:
            ...
            # This means all available results are traversed; stop further tasks
            self.errors.append(f"There are no more pages to crawl, last url: {url_to_crawl}")
            async with self.lock:
    ...
    async def crawl_custom_query(self):
        ...
        line_print(Back.GREEN, f"Finish crawling {self.base_url} with max_depth of {self.max_page} pages")
        ...
        try:
            ...
            async with self.lock:
                if url_destination in self.page_data:
                    raise Exception(f"url {url_destination} is already in page_data, Skipped!")
    ...
    async def crawl_page(self, url_to_crawl):
        try:
            ...
            await self.add_page_visit(url_to_crawl)
            ...
            if get_url_path(url_to_crawl, segment=0) in ['', '/']:
                async with self.lock:
                    self.page_data[url_to_crawl] = page_info

                new_urls = [item['link'] for item in page_info if 'link' in item]  # type: ignore
...
async def scrape_nyaa_with_query(base_url, max_pages):
    async with AsyncScraper(base_url, max_concurrency=3, max_page_traverse=max_pages) as crawler:
        ...
        line_print(Back.RED, "Here are the errors that occurred during crawling: ")
...
def run_scraper(query: str, pages: int, output: str, latest: bool) -> dict | None:
    """Run the Nyaa scraper and return the data dict (or None on failure)."""
    crawl_datas = None
    if latest:
        print("Fetching latest data")
        crawl_datas = asyncio.run(scrape_nyaa_latest("https://nyaa.si", pages))
    else:
        print("Fetching with query")
        base_url = generate_nyaa_url(query=query, category="1_2")
        crawl_datas = asyncio.run(scrape_nyaa_with_query(base_url, pages))
    ...
    if output == "both":
        ...
        print(Fore.GREEN + f"data is stored in file {name}")
    elif output == "file":
